...
tray_update(Tray *tray)
{
	...
	int padding = 3; /* Отступ сверху и снизу в пикселях (увеличьте, чтобы сделать иконки еще меньше) */
	...
	icon_size = tray->height - (padding * 2);
	if (icon_size < 1)
		icon_size = 1;
	...
	wl_list_for_each(item, &watcher->items, link) {
		...
		if (item->icon) {
			...
			pixman_image_composite32(PIXMAN_OP_OVER, img, NULL,
			                         canvas, 0, 0, 0, 0,
			                         slot_x_start, padding, canvas_width,
			                         canvas_height);
